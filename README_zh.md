...
Notion 是普通连接流程中的可选服务。进入**设置 → 连接 → 添加连接**，选择
**Notion**，再点击**授权并添加**。只有执行这些操作后，Wisp 才会在浏览器中
打开 Notion 授权页；启动应用时不会自动创建或认证 Notion 连接。

连接使用官方 `https://mcp.notion.com/mcp` 端点；OAuth 访问令牌和刷新令牌保存在
系统密钥环中，不会写入项目数据库。只有授权成功后才会保存连接，删除连接时也会
清除对应凭据。
