...
- Each registered context can open independent ephemeral interactive terminals in a resizable, tabbed bottom dock. The `+` menu selects any registered context, including one already used by another live tab. Local, WSL, and system-OpenSSH launch adapters share one PTY-backed TerminalSession manager; closing the view detaches it, while explicit termination ends the active process. Terminal buffers are not persisted and tracked computation continues to use Runs.
